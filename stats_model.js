...
/**
 * advanced_stats.js  v3
 *
 * Data layer. Merges:
 *   nba_service.py  — official NBA: ORtg/DRtg/NetRtg/Pace/PIE/clutch/hustle/
 *                     shot quality/players/on_off splits (for injury model)
 *   BallDontLie     — H2H current season
 */
...
async function fetchH2H(homeId, awayId) {
  ...
  const k = `bdl:h2h:${homeId}:${awayId}:${season}`; const h=cg(k); if(h) return h;
  try {
...
function mergeTeam(nba, form) {
  ...
  return {
    ...
    ftr:sn(p.ftr,0.22), tov_pct:sn(p.tov_pct,14), ast_to:sn(p.ast_to,2.0), ast_pct:sn(p.ast_pct,0.60),
    oreb_pct:sn(p.oreb_pct,0.24), dreb_pct:sn(p.dreb_pct,0.76),
    pts:sn(p.pts,110), reb:sn(p.reb,44), ast:sn(p.ast,26), stl:sn(p.stl,8), blk:sn(p.blk,5), tov:sn(p.tov,14),
    fga:sn(p.fga,88), fgm:sn(p.fgm,42), stl_rate:sn(p.stl,8)/poss*100, blk_rate:sn(p.blk,5)/Math.max(sn(p.fga,88),1),
    clutch_w_pct:sn(p.clutch?.w_pct,sn(f.win_rate,0.5)), clutch_plus_minus:sn(p.clutch?.plus_minus,0),
    clutch_fg_pct:sn(p.clutch?.fg_pct,0.45), clutch_ft_pct:sn(p.clutch?.ft_pct,0.77), clutch_tov:sn(p.clutch?.tov,14),
    hustle_score:sn(p.hustle?.score,20), contested_shots:sn(p.hustle?.contested_shots,15),
    contested_3pt:sn(p.hustle?.contested_3pt,7), charges_drawn:sn(p.hustle?.charges_drawn,0.5),
    screen_ast_pts:sn(p.hustle?.screen_ast_pts,20),
    defense:p.defense||{},
    star_power:sn(p.star_power,15), top_pie:sn(p.top_pie,0.12), top_usg:sn(p.top_usg,0.28),
    ...
    win_rate10:sn(f.win_rate10,sn(f.win_rate,0.5)), avg_diff:sn(f.avg_diff,sn(p.net_rating,-2)*0.4),
    avg_diff5:sn(f.avg_diff5,0), avg_diff10:sn(f.avg_diff10,0),
    momentum:sn(f.momentum,0), streak:sn(f.streak,0),
    avg_pts:sn(f.avg_pts,sn(p.pts,110)), avg_pts_allowed:avgAllowed,
...
function buildDeltas(home, away, nd) {
  ...
  const homeHWR=home.home_win_rate??home.win_rate, awayAWR=away.away_win_rate??away.win_rate;
  return {
    ...
    split_prob: clamp(((homeHWR??0.5)+(1-(awayAWR??0.5)))/2,0.25,0.75),
    home_home_wr:homeHWR, away_away_wr:awayAWR,
...
async function getAdvancedMatchup(homeTeam, awayTeam) {
  const [nba, homeId, awayId] = await Promise.all([
  ...
  const matchup  = buildDeltas(homeData, awayData, nba?.deltas||null);
  return {
    matchup, homeData, awayData, homeId, awayId, h2h,
    homeOnOff: nba?.home_on_off || [],
    awayOnOff: nba?.away_on_off || [],
    dataSource: { nbaServiceAvailable:!!nba, bdlH2HAvailable:h2hGames.length>0 }
  };
}
